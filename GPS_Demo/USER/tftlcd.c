...
extern u8 flag_draw;
extern u8 flag_demo;
extern char lon_str_end[15];
extern char lat_str_end[15];
//LCD的画笔颜色和背景色	   
...
u16 y0=y;   
    ...
    {   
if(t<24)temp=tfont24[index*3][t];           //?24???
else if(t<48)temp=tfont24[index*3+1][t-24]; //?24???                          
        else temp=tfont24[index*3+2][t-48];         //?24???
   for(t1=0;t1<8;t1++)
{
if(temp&0x80)LCD_DrawFRONT_COLOR(x,y,color);//????
...
void Interface_Display(void)
{
	...
	LCD_ShowString(40,20,300,24,24,"DIY_Final Demo");	
	LCD_ShowString(40,50,300,24,24,"Model  :STM32F4");	
	LCD_ShowString(40,80,300,24,24,"Author:");
	show_font();//显示中文名字
	//LCD_ShowString(40,110,300,24,24,"Locating failed");   	 
...
void Gps_Msg_Show1(void)
{
  ...
  //sprintf((char *)dtbuf,"%02d/%02d/%02d,%02d:%02d:%02d",year-2000,month,day,hour,gpsx.utc.min,gpsx.utc.sec);//显示北京时间
	...
	//printf("year2:%d\r\n",gpsx.utc.year);
	LCD_ShowString(60,370,260,24,24,dtbuf);		
...
{		
	...
	//Interface_Display();
	FRONT_COLOR=BLUE;  	 
	//sprintf((char *)dtbuf,"Longitude:%.5f %1c   ",Save_Data.longitude,Save_Data.E_W);	//得到经度字符串
 LCD_ShowString(40,160,200,24,24,"Longitude:");	
	LCD_ShowString(180,160,200,24,24,lon_str_end);	

  LCD_ShowString(290,160,200,24,24,Save_Data.E_W);	
		//delay_ms(50);
 	LCD_ShowString(40,190,200,24,24,"Latitude:");	

	LCD_ShowString(180,190,200,24,24,lat_str_end);	
  LCD_ShowString(290,190,200,24,24,Save_Data.N_S);	
	//delay_ms(50);	

	LCD_ShowString(40,220,200,24,24,"UTC Time");	
	//strncpy(hour, Save_Data.UTCTime, 2);
	LCD_ShowString(180,220,200,24,24,Save_Data.UTCTime);	
//	LCD_ShowString(205,220,200,24,24,":");
	//delay_ms(50);
